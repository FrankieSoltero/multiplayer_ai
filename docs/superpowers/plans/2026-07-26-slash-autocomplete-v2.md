...
- **Final review (2026-07-26):** Spec §3b's claim that filtering at `derive.ts` cleans "every consumer" overlooked the SKILL SUITE path (`skillSuite.ts` consumes `ProjectSessionInfo.skills` raw, bypassing `DerivedState`). Fixed by importing `notRemoved` in `skillSuite.ts` and filtering in `suiteFromSessions`, with a new test in `skillSuite.test.ts` verifying that suite output excludes `(removed)` entries (+1 test, count now 57).
