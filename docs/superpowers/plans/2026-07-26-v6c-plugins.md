...
- **Task 7 known unknown — CONFIRMED at the manual demo (2026-07-26):** the SDK's live `supportedCommands()` roster DOES include non-skill slash commands. After importing soltero-skills the fresh-session roster refreshed to 71 entries: built-ins + namespaced plugin skills + junk rows such as `/agents` with description "(removed)…". No filter was added at demo time per the plan's "don't pre-filter blind" instruction — the filter question was put to the user ("filter built-in commands out of the roster?") and is UNANSWERED. Note for whoever implements a filter: the SDK `SlashCommand` type has no skill-vs-command discriminator (sdk.d.ts:6596-6613), so any filter is heuristic (e.g. by description text or a known-builtins denylist). Folded into the slash-autocomplete-v2 brainstorm as an open question.
