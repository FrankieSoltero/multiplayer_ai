...
## 🚀 START HERE (2026-08-06, Claude Code — lab codification sweep on `docs/home-lab-codify`, PR pending) — the 2026-08-05 lab's kink ledger is CODIFIED. Shipped on this branch: `deploy/hub/setup.sh` phase 1 gains `build-essential` (kink 4) + REPO_URL-as-the-mpai-user header warning (kink 3); **`deploy/home-lab.md` NEW** (WSL2 LAN-mode hub: systemd, keepalive, mirrored-networking/portproxy, LAN env edits, insecure-context note); `deploy/multi-machine-test.md` §2 documents `AGENT_PLUGINS_ROOT` (kink 6) + the manual plugin-registry workaround for the v7b3 deferral (kink 7), §6 results table records the partial 2026-08-05 run (PASS: 9, 11b · PARTIAL: 1, 2, 11c · DEFERRED: 7, 11a · rest pending the 3-laptop matrix); `docs/mistakes-and-fixes.md` +4 lessons (randomUUID secure-context, build-essential, mpai-user REPO_URL, WSL2 keepalive); `docs/tech-debt.md` §2.11 NEW (end_session gap kink 8 — user-requested feature, misleading SkillsPanel message, "attach a repo first" empty-state, canUseTool log noise). **`.soltero/` is now actually gitignored** (it never was, despite ledger claims — leak hazard on a now-PUBLIC repo) and the lab box's IPs/ssh/username moved OUT of this file into untracked **`.soltero/lab-box.md`** (repo is public; HANDOFF is committed — keep machine details there, not here).

**LAB STATE (2026-08-06, teardown complete):** everything SHUT DOWN cleanly per the runbook teardown — all Mac mpai processes killed (incl. the user's global `~/.local/bin/mpai` that had self-attached), hub `systemctl stop`ped (verified inactive), record KEPT on the box (hub.db + wal ~1.2MB + 2 backups). Box stays reachable (keepalive + tailscale). VERIFIED LIVE 2026-08-05: OAuth round trip · pairing (code flow + silent bearer re-attach) · attach-truth lines (§10.4a) · project+repo attach+session create · setup.sh first real run. Hub ran auth ON, sqlite store (verified not in-memory), boot backup taken, ANTHROPIC_API_KEY unset (oversight deferred — user has no key).

**Resume the lab:** exact box details + 3-step resume commands are in `.soltero/lab-box.md` (local only). In short: start the unit over ssh → relaunch the Mac machine with `AGENT_PLUGINS_ROOT` set (silent bearer re-attach expected) → fresh session in project `lab-name`, then runbook items 3-5 + 10 and keep filling `deploy/multi-machine-test.md` §6. If `.soltero/lab-box.md` is missing (fresh clone), ask the user for the box's address — do NOT re-derive from git history.

**NEXT (in order):** (1) push this branch + open PR (user merges — PR-first house rule). (2) Resume the lab per above; finish runbook items 3-5, 10, then the full 3-laptop matrix (blocked on the user picking lab repos 2-4). (3) Feature work when the user picks it: `end_session` (tech-debt §2.11, user-requested — needs lean-brainstorming, it's a product-semantics call) is the top candidate.

**Open questions for the user:** license pick (repo is PUBLIC with no LICENSE — MIT vs Apache-2.0 vs AGPL, asked, unanswered); ANTHROPIC_API_KEY for hub oversight (user has none yet — runbook item 7 deferred); which repos become lab repos 2-4 for the full 3-laptop matrix.

## 🚀 START HERE (2026-08-05 late) — PR #42 MERGED (main `756221a`): finishing cycle + README rewrite + root-PNG cleanup + `deploy/hub/setup.sh` (universal one-command hub provisioning) all on main. Zero open PRs. **IN PROGRESS: home-lab test setup on the user's Windows PC** — plan: Ubuntu-24.04 on WSL2 as the hub box (systemd=true in /etc/wsl.conf DONE + wsl --shutdown DONE; next: verify systemd, then mirrored networking via %UserProfile%\.wslconfig `networkingMode=mirrored` on Win11 22H2+, else netsh portproxy 4000 + Defender inbound rule), then `sudo HUB_HOSTNAME=<PC-LAN-IP> SKIP_CADDY=1 SKIP_UFW=1 bash deploy/hub/setup.sh` from a clone, then env edits for LAN (HOST=0.0.0.0, http:// forms of HUB_ORIGIN + OAUTH_CALLBACK_URL at <PC-LAN-IP>:4000, GitHub OAuth app registered against those, ANTHROPIC_API_KEY), start unit, pair the 3 laptops per `deploy/multi-machine-test.md` (TLS/ufw items deferred in LAN mode). After the lab proves it: codify as deploy/home-lab.md. This HANDOFF edit is uncommitted on main — fold into the next docs commit.

## 🚀 START HERE (2026-08-05, Claude Code #1 cont. — finishing cycle IMPLEMENTED, 3 commits, reviews clean so far) — §8.8 MERGED (PR #41 → main `0fd9cdb`). FINISHING CYCLE (user /goal) SHIPPED on `feature/prd-finishing`: §10.4's two v7b1 residuals are RESOLVED — (a) `228c432`/`384d924` join success signal (hub + solo `{type:"joined"}` ack sent before replay, client Header renders `● JOINED`); (b) `3609c16` hub URL normalization (bare `--hub` origin gains `/uplink`) + welcome-gated attach truth (`dialing hub <url> …` at launch, `attached to hub` only after the relay's `onAttached` fires on a real `welcome`, loud once-per-process `onNoWelcome` warning on silence). Docs wrap (this task, uncommitted at write time): `deploy/multi-machine-test.md` runbook created (topology, env, 14-item checklist, teardown, failure-recovery, results table — the vehicle for the §8.10 real-box test); `docs/PRD.md` §10.4 both residuals marked resolved, §8.10 *Today* now points at the runbook. **NEXT: final whole-branch review (lean-sdd) → soltero-skills:lean-finishing → PR for user review → the user's 3-computer test, run against `deploy/multi-machine-test.md`.** PRD state: §8.1–8.9 ALL at final state, §10.4 residuals resolved; §8.10 code-done, real-box verification = the runbook-driven 3-computer test; §8.6 backlog + local-models + fine-tuning parked (tech-debt §2.9, §2.10 accepted minors).

## 🚀 START HERE (2026-08-04, Claude Code #1, updated late-session) — §8.8 HUB-SIDE OVERSIGHT: ALL 5 TASKS IMPLEMENTED, REVIEWS CLEAN SO FAR.

**State:** branch `feature/hub-oversight` off main (base: PR #40 permission-rules MERGED `56296d3`; PR #16 CLOSED; zero open PRs). **Spec USER-APPROVED:** `docs/specs/2026-08-04-hub-oversight-design.md` (R1 hub-host `ANTHROPIC_API_KEY` capability, truthful `available:false` · R2 member-gated toggle, lifecycle-byte-identical refusal · R3 HubDb schema v4 `project_oversight` persists enabled+summary+seq · R4 `oversight_update` down-frame to uplinks, injection precedence hub-pushed-over-local); plan `docs/plans/2026-08-04-hub-oversight.md`, plan-review `docs/plan-reviews/2026-08-04-hub-oversight-review.md`. All 5 plan tasks (hub Overseer + capability + persistence, `set_oversight`/down-frame wiring, laptop injection precedence + `team_update` parity, client unavailable rendering, docs sweep) are implemented via lean-sdd's pipelined subagent flow, each reviewed clean so far. Docs sweep (this task) landed PRD §8.8 *Final state* → reached (hub-side oversight section rewritten to describe shipped behavior, the "still open — split ruling" phrasing removed) and this HANDOFF update; `docs/tech-debt.md` had no "hub-side oversight still open" line to resolve (checked — none existed). **NEXT:** final whole-branch review (lean-sdd) → `soltero-skills:lean-finishing` → PR (user merges). If resuming fresh: spec + plan + reviews are the contract; nothing else in §8.8 is open.

## (was) ALL PRs MERGED (user-directed): #38, #33, #35, #34, #36, #37, #39 are in `main` (`d09cd1e` merged last); only #16 stays open (stale, recommend close). Branch audit ran post-merge: NO lost work — every other branch is fully contained in main; the only unmerged commits anywhere are `feature/permission-rules`' own two + #16's doc. **`feature/permission-rules` now needs a rebase onto the new main** (it sits on the pre-fix local-models tip; expect a GateBar.tsx conflict with the uncommitted client work). #33 was merged from draft at user direction — its live-walk gate (both-theme screenshots) never ran; record that as an accepted skip, not done. **LOCAL-MODELS SETUP — PROVEN, then PARKED at user direction (2026-08-04).** The wiring works end-to-end: `~/.mpai/litellm/litellm.config.yaml` was repointed to the actually-pulled model (`qwen3.6-27b` → `ollama/qwen3.6:27b`); the proxy booted on :4000 and an Anthropic-format `/v1/messages` call through it returned `LOCAL MODEL OK` from the local model (thinking model — needs max_tokens ≥ ~300, reasoning eats small budgets and content comes back empty). Everything then SHUT DOWN at user request: proxy, demo server (:3001), vite (:5173) all killed, model unloaded (`ollama stop`); Ollama daemon itself left running (predates us). The in-UI live walk (session + gate + screenshot, plan done-gate 4) was started but NOT completed — user interrupted at project creation; still the recorded deferral. **Facts for whoever resumes:** (1) This Mac is 24GB unified (M5) — the 17GB `qwen3.6:27b` runs but squeezes it; user's PC (16GB VRAM + 64GB RAM) is the intended local-model host later (Ollama there, LiteLLM local entry → `http://<pc-ip>:11434`; `qwen3:14b` full-GPU or a 30B-A3B-class MoE w/ expert offload fit it well). (2) Proxy env has no `ANTHROPIC_API_KEY` → cloud pass-through 401s through the proxy; local ids need no key. (3) Fine-tuning discussion parked too — base of choice if resumed: Qwen3.6-27B (Apache 2.0), training data = harvested hub session records. (4) A clean built main worktree existed at scratchpad `wt-demo` (session-scoped, likely gone).

## (was) PR SPOT-CHECK SWEEP DONE, 4 FIXES SHIPPED; `feature/permission-rules` IS FURTHER ALONG THAN THE BLOCK BELOW SAYS.

**Permission-rules TRUE state (the block below is stale on this):** the plan is committed (`f8852c1`) and the SERVER side of §8.6 cycle 2 landed as `cb58775` (session-scoped `updatedPermissions` + gate enrichments on the wire). The CLIENT side exists but is **UNCOMMITTED in this working tree** (~332 lines across `App.tsx`, `GateBar.tsx`+test, `Transcript.tsx`+test, `derive.ts`+test, `types.ts`, `terminal.css`, plus a small `deploy/local-models.md` edit) — no SDD ledger covers it, no PR exists for the branch. **First job of the next work session: reconcile that diff (review it, commit it path-scoped, or finish it), then PR.** Note `tetris.test.ts`'s 2-line working-tree diff is just the TS6133 import fix, now superseded by PR #38 — safe to discard once #38 merges.

**PR sweep (2026-08-04, all 6 open PRs spot-checked by subagents, fixes applied by subagents in isolated worktrees — this checkout untouched):**
- **PR #38 NEW** `fix/tetris-unused-imports` off main (`93654d5`): main's client `tsc -b` was BROKEN (TS6133, unused `normalize`/`SHAPES` in `tetris.test.ts`, since `b600384`). One-line fix; user merges.
- **#37** local-models: `35e5dd1` — `parseExtraModels` reserved-id (`__proto__`/`constructor`/`prototype`) entries now warn-and-skip (`Object.create(null)` + explicit check); server **803**, tsc clean.
- **#36** agent-surface: `7051df0` — stopTurn race fixed: `outcome "interrupted"` only when the interrupt flag is set AND the result is error-shaped; a turn completing cleanly in the race window keeps truthful "success"+usage/cost. sdk.d.ts:3457-3461 confirms interrupted turns always surface error-shaped, so the tie-break is sound. Server **791**, tsc clean.
- **#33** presentation (draft): `c0a9487` — GateBar jump now keyboard-reachable (role/tabIndex/Enter/Space, parity with subagent rows) + double-space cosmetic fix; client **492**. Draft still gated only on the live-walk (both-theme screenshots).
- **#34** lifecycle: body addendum only (two omitted ride-along commits `e00635e`/`f41023a` disclosed; false "tsc ×3 clean" claim corrected → points at #38). No code change.
- **#35** invites: CLEAN. One un-fixed nitpick on record: dead `invite_created`/`invite_revoked`/`invite_redeemed` code in `poc/server/src/events.ts:55-57` + `Transcript.tsx:312-327` (nothing emits them anymore).
- **#16** landing-page-context (draft): stale — main moved ~249 commits past it and it cites uncommitted `market-research.md` as its source of truth. **Recommended CLOSE; awaiting user's call.**

**Stack note:** `feature/permission-rules` sits on the OLD tip of `feature/local-models` — it does not contain `35e5dd1` (#37), `7051df0` (#36), or `c0a9487` (#33). Rebase/merge-up when the stack next moves; conflicts unlikely except GateBar.tsx (the uncommitted client work touches it, and #33's `c0a9487` touched it on its own branch).

---

## (was) START HERE (2026-08-03, Kimi CLI #5) — LOCAL MODELS (PATH 3, shim routing) **SHIPPED** on `feature/local-models` (stacked on `feature/agent-surface`/PR #36; plan `docs/plans/2026-08-03-local-models-shim.md`).
...
**Suites at head:** server **789** · hub **368** · client **566**, tsc ×3 clean. **PR #36 open** (stack: #33 ← #35 ← #34 ← #36).

**Facts worth keeping:** (1) `agent_error` is appended BEFORE its `turn_end` so record.ts groups the failure with the turn. (2) `system/api_retry` folds into `agent_status` "retrying" — one busy-state channel, not a second event type. (3) New plugin PATHS bind only in sessions created after the add (pre-existing bound, documented in `AgentDriver.reloadPlugins`); reload hot-refreshes skills of known plugins. (4) The `supersedes` flag's semantics are "this frame replaced earlier refused content" — the client marks the FLAGGED delta retracted per the T6 brief; recorded here so the semantic is settled one way. (5) `rate_limit.resetsAt` units are SDK-unspecified — client reads ms-if->1e12-else-seconds; confirm against a real event once a successful turn can run. (6) `sessionCostUsd` is derived+tested but has no HUD surface yet — natural home is the future /usage cycle.

**Live-proof record (2026-08-03):** turn_end payload ✓, stop_turn → outcome "interrupted" ✓, error truth ✓ — and it caught a real bug: API failures arrive as result subtype "success" + is_error + terminal_reason, which used to render "turn failed (success)"; fixed in `0b4a0b2`. **Two environment traps for whoever re-runs it:** (a) the user's own mpai daemon holds IPv6 *:3001 (PID from Aug 1, `mpai --hub ... --project tester`) — `localhost:3001` resolves IPv6-first, so scripts and the vite client silently talk to THEIR daemon (pre-agent-surface code), never ours; run demo servers on another port and use explicit 127.0.0.1. (b) The Claude account behind the CLI credentials hit its WEEKLY limit (resets Aug 4 ~7pm ET) — a successful turn with nonzero usage can't run until then; every turn fails `api_error` in 300ms. Demo server left running on :3005 (agent-surface code, auth off, AGENT_WORKDIR_ROOT=poc/demo-worktrees — sessions must reuse the pre-made ana/ben worktrees; the standalone server does NOT provision).
...
**Also this session:** PR #30 (§8.10) merged `1710b23` + PR #31 (worktree ref-namespace fix) merged `0a5ae4a`, both user-directed; suites on main server 750 · hub 350 · client 391; Branch-B SDD workspace deleted. Live demo state still running (user may want it torn down): dev hub :4000, dev-demo :3005, demo-buddy :3006 (scratch repo in job tmp, session `frankie` in project `test` — the "user-ea8a" identity in the transcript is OUR Playwright browser, whose visible window the user also clicked around in), user's daemon :3001 attached to the dev hub. User's checkout still has legacy branch `mpai/test` + worktree `.mpai/worktrees/test` blocking project `test` sessions on THIS repo (their call to clear: `git worktree remove .mpai/worktrees/test && git branch -D mpai/test`). Screenshots session-frankie-*.png at repo root, untracked.

## 🟡 (superseded this session) previous session-#26 block below

## (was) START HERE (session #26, post-merge) — ALL MERGED, ON `main` (`0a5ae4a`), NOTHING IN FLIGHT. PR #30 (Branch B §8.10) merged `1710b23` and PR #31 (worktree ref-namespace fix) merged `0a5ae4a`, both user-directed this session. Fresh suites on main: **server 750 · hub 350 · client 391**, tsc ×3 clean. SDD workspace `.soltero/lean-sdd/2026-07-31-operating-a-hub/` DELETED (house precedent). This HANDOFF edit is uncommitted — fold into the next branch's first docs commit as usual. **NEXT: the user picks the next PRD §8 section** (remaining: §8.4 sub-sessions — the last *mechanism* section, recommended; §8.5 remainder + §8.9 themes pair naturally as one presentation cycle; §8.6 needs a Claude-Code gap inventory as its first task; §8.8 hub-side oversight second half). Live demo state from this session (leave or tear down as the user prefers): dev hub :4000, dev-demo laptop :3005, demo-buddy laptop :3006 (scratch repo `$CLAUDE_JOB_DIR/tmp/demo-buddy-repo`, session `frankie` in project `test`), user's own daemon :3001 attached to the dev hub with `--project test`. The user's checkout still carries legacy branch `mpai/test` + worktree `.mpai/worktrees/test` — blocks sessions in project `test` on THIS repo until the user clears it (their call). Screenshots session-frankie-*.png at repo root, untracked.

### ✅ FIXED (session #26) — session-create ref-namespace collision → **PR #31 OPEN — DO NOT MERGE, the user does** (https://github.com/FrankieSoltero/multiplayer_ai/pull/31)

Diagnosed via lean-debugging (scratch-repo repro FIRST, exact error reproduced), fixed test-first via lean-tdd on branch `fix/session-branch-namespace-collision` (cut from `origin/main` `ddc8432`, single commit `26fe51b`). Root cause confirmed: git's ref store cannot hold a ref at a path AND refs below it — legacy flat-scheme branch `mpai/test` (pre-§8.2, created 2026-07-27, **in THIS checkout**, still checked out at `.mpai/worktrees/test`) blocks every `mpai/test/*` branch, and `provision()` checked only the exact branch, leaking raw git stderr. Fix: `namespaceBlocker()` pre-flight in `provision()` (both directions: prefix ref above the target, any ref below it) → clear refusal naming the blocker. 2 new tests (both watched RED with the exact raw-stderr leak). Suites at `26fe51b`: **server 750 · hub 266 · client 390**, tsc ×3 clean.

**Disclosed in PR #31, still true:** (1) alternative fix (collision-proof naming scheme) NOT taken — spec change + worktree migration; it's a product call if wanted. (2) **This repo still has the collider** — with the fix the UI shows a clean refusal, but project `test` stays blocked until the user clears it: `git worktree remove .mpai/worktrees/test && git branch -D mpai/test`. Left untouched (user's worktree — never delete unasked).

### Session #25 also shipped (after PR #30 opened) — dev-run findings + user-directed ride-along

- **Dev stack was run live** (skill `run`): hub on 4000 (scratch `MPAI_HOME=~/.claude/jobs/4026c7bb/tmp/devrun/mpai-home`, retention 30d + backups on — boot lines + boot backup file verified), machine `dev-demo` attached into project `branch-b-demo`, screenshot `devrun-machine-online.png` (repo root, untracked). Processes may still be running in background (hub task bbdpuhf3h, laptop bvd56ze75); ports 4000/3005. **User's own daemon on 3001 untouched; user's Chrome tab was ALSO connected to the dev hub** (it re-registered their "Recall AI" project into the scratch store — harmless, scratch was wiped once already at user request).
- **Found live: bare `--hub ws://host:4000` URL silently registers NO machine** — relay passes the URL verbatim (`poc/server/src/relay.ts:225`), hub routes uplinks by URL path (`poc/hub/src/hub.ts:816`), so without `/uplink` the socket lands on the browser arm; "attached to hub" still prints. Canonical URL: `ws://host:port/uplink`.
- **Ride-along commits pushed to PR #30** (user-directed quick fix, disclosed in PR body "Ride-along" section): `b240e0c` — `refusalText` gains ctx param (`poc/client/src/projectAccess.ts:19-31`), SessionPicker passes real `ws(s)://host/uplink` + projectId (`poc/client/src/components/SessionPicker.tsx:395-407`), test added (`projectAccess.test.ts:69-77`), client 390→391. `f030284` — `deploy/hub/RUNBOOK.md:78` pairing command corrected to `wss://…/uplink` (CLI rejects https:// and needs the path).
- Candidate follow-up flagged to user (not done, design call): CLI/relay could normalize a bare hub origin by appending `/uplink` so the silent half-attach can't happen.

**Final state:** all 9 tasks of `docs/plans/2026-07-31-operating-a-hub.md` executed and reviewed via lean-sdd on `feature/operating-a-hub` (branch tip now `f030284` = 9 tasks `cba9c62..437512d` + HANDOFF wrap `4d4a01a` + ride-along `b240e0c`/`f030284`, all pushed; THIS HANDOFF edit is uncommitted — fold into the next docs commit as usual). One fix round total (Task 8 RUNBOOK sentence, re-review clean); every other task review clean. Final whole-branch review (opus): **Ready to merge, 0 Critical, 0 Important**; all deferred minors triaged ACCEPT and disclosed in PR #30's body. Fresh suites at head: **server 748 · hub 350 (was 266) · client 390**, tsc ×3 clean. SDD workspace (`.soltero/lean-sdd/2026-07-31-operating-a-hub/`) KEPT until merge, then delete (house precedent). **On merge of #30:** delete that workspace; next section is the user's pick (PRD §8 remaining: §8.4 sub-sessions, §8.5 presentation, §8.6, §8.8 hub-side second half, §8.9 themes). Open owner items: staticFiles.ts security headers (still Caddyfile-only — flagged as possible follow-up in PR #30 disclosure 2); L7 no-LICENSE (user deferred, ask first); deploy/hub artifacts are UNVERIFIED until a real box exists.
...
**Suites on main:** server 725 · hub 195 · client 361, tsc ×3 clean. SDD workspaces deleted.

**Security audit of record:** `Docs/audit-2026-07-30.md` (0 critical / 0 high / 6 medium / 8 low). FIXED: M2 digest sanitization, M3 symlink-aware containment, M5 server maxPayload, M1/M6 plugin clone allowlist. STILL OPEN: **M4** (hub has no auth/TLS/fail-closed HOST check — this IS §8.1+§8.10, not a patch); **L7** no LICENSE file (user deferred the license choice — ask before adding one).

**NEXT SECTION — user picks. Recommendation on record: §8.1 Identity & access + §8.10 Operating a hub as a PAIR.** Rationale: they are the D12 exposure gate; the audit's only unremediated medium (M4) is exactly them; and §8.10 owns retention/compaction/backup for §8.7's now-unbounded journal (tech-debt §2.8). Alternatives: §8.8's second half (hub-side oversight, split out by spec §8a.1 and still open), §8.6 (needs a Claude-Code gap inventory as its first task), §8.4 sub-sessions, §8.9 presentation/themes, §8.5 (presentation polish only — closest to done).

**Pipeline (unchanged, worked twice):** lean-brainstorming → spec → lean-plans → plan-review (0.19.1 severity gate: ≥85 + floors ≥80 + zero blockings) → lean-sdd (serial per the conflicts table; impl mechanical=sonnet else opus; reviews haiku/sonnet/opus by tier; controller=fable never dispatched) → live walk → final review → ONE fix wave → push → **open PR, never merge without asking**.

**Standing gotchas:** never `git add -A` (user WIP: `poc/client/src/game/tetris.test.ts` + untracked market-research.md, poc/demo-plugins/, screenshots, .soltero/); client typecheck `npx tsc -b`; hub/client verifies must build poc/server FIRST (pretest doesn't fire under `npx vitest run`); grep is aliased to ugrep (`command grep`); user's own `mpai` daemon may hold port 3001 — never kill it; walk scratch repos need an init commit; route fix rounds to the IMPLEMENTER agent, never a reviewer (twice-made error, lesson in docs/mistakes-and-fixes.md).

## 🟡 HISTORICAL — session #20 state below (PRs #22/#23 merged; §8.7 section pick was pending — now made: §8.7, this branch). Superseded where it conflicts with the block above.

## (was) START HERE — PRs #22 AND #23 ARE **MERGED** (user's call, executed session #20). `main` = `af63bec`, we are ON main, suites verified there (server 499 / hub 104 / client 279*). Next work: the user picks the next PRD §8 section, and the loop runs on **soltero-skills, NOT superpowers** (user directive, see below).
