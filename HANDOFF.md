...
## 🚀 START HERE (2026-08-05, Claude Code #1 cont.) — §8.8 MERGED (PR #41 → main `0fd9cdb`). FINISHING CYCLE IN FLIGHT (user /goal): fix the two §10.4 v7b1 residuals — (a) silent uplink failure: wrong/bare hub URL indistinguishable from attached (relay.ts passes URL verbatim, hub routes by /uplink path — the live-found half-attach trap, HANDOFF session #25); (b) relay join emits no success signal — then a 3-computer test RUNBOOK (topology: hub + 3 machines; repos 1+2 / 2+3 / 3+4 — repos 2,3 deliberately shared = §8.8 collision demo), then PR for user review. User tests on 3 machines after review. Pipeline: spec+plan (user pre-delegated approval to PR review per /goal) → council gate → lean-sdd → PR. PRD state: §8.1–8.9 ALL at final state; §8.10 code-done, real-box verification = the upcoming 3-computer test; §8.6 backlog + local-models + fine-tuning parked (tech-debt §2.9, §2.10 accepted minors).
