...
## 🚀 START HERE (2026-08-04, Claude Code #1, updated late-session) — §8.8 HUB-SIDE OVERSIGHT CYCLE IN FLIGHT.

**State:** PR #40 (permission-rules) MERGED (`56296d3`); PR #16 CLOSED; zero open PRs; main = post-#40. PRD remainder surveyed: §8.8 hub-side oversight is the last feature build (user picked it), then §8.10 real-box verification (needs a box) and the §8.6 parity backlog (tech-debt §2.9). **Spec USER-APPROVED:** `docs/specs/2026-08-04-hub-oversight-design.md` (R1 hub-host `ANTHROPIC_API_KEY` capability, truthful `available:false` · R2 member-gated toggle, lifecycle-byte-identical refusal · R3 HubDb schema v4 `project_oversight` persists enabled+summary+seq · R4 `oversight_update` down-frame to uplinks, injection precedence hub-pushed-over-local). User directive via /goal: **finish the whole cycle** — lean-plans → plan-review gate → lean-sdd → lean-finishing → PR (user merges). If resuming fresh: spec is the contract; branch `feature/hub-oversight` off main; first docs commit = spec + plan + review + HANDOFF edit (house convention).
