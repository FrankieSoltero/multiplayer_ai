...
## 🚀 START HERE — §8.8 Awareness (collisions half) is IN FLIGHT on branch `feature/awareness-collisions` (cut from main `6ba1ee2` = §8.7 merged via PR #26, user's call). **Spec APPROVED by user 2026-07-30:** `docs/specs/2026-07-30-awareness-collisions-design.md` — read it first; its §8a rulings 1-3 (scope split: collisions only, hub-side oversight is a SEPARATE later branch; freshness = turn boundaries + pre-gate; debt §2.1 fixed at root via project-scoped worktrees) and its §2 banked session-#11 decisions are LOCKED. Pipeline position: plan WRITTEN + round-1 council BLOCKED 82.3 (7 blockings, D6 78 — legitimate under BOTH gates) + ALL 33 findings fixed (fix report .soltero/plan-fix-round1-report.md) + spec §8a rulings 4-6 added (kill switch MPAI_CONTESTED_GATE=0; touched-broadcast exposure accepted; richer contested frame with collisions[]). **GATE CHANGED: soltero-skills 0.19.1 shipped the CC-001 fix — gate is now overall ≥85 AND floors ≥80 AND zero blocking violations; CC-001 retired in corrections-ledger. ROUND 2 MUST USE THE 0.19.1 SCRIPT PATH** (~/.claude/plugins/cache/soltero-skills-marketplace/soltero-skills/0.19.1/skills/plan-review/...). Review report: docs/plan-reviews/2026-07-30-awareness-collisions-review.md. Next: **round 2 council (0.19.1)** — on PASS, → plan-review gate (≥95; the user's standing override lever for minors-only BLOCKs applies — precedent in docs/plan-reviews/2026-07-29-the-record-review.md; gate-calibration ledgered as docs/corrections-ledger.md CC-001 for end-of-month skill-patcher) → lean-sdd execution → push + OPEN PR (never merge; user merges).

Key design facts for the planner: signal = `touched: string[] | null` on SessionFacts (repo-relative, git-derived diff+status vs merge-base, TOUCH_CAP 500 + "…" sentinel, laptop-computed at turn end + pre-write-gate; additive facts field, NO protocol bump — journal persists it free); pure `collisionsFrom()` in poc/server beside record.ts exported as multiplayer-ai-server/collisions; surfaces = calm-colour `⚠ CONTESTED ▸ N` header badge (amber reserved for gates) + per-repo chip + PartyPane row lines; agent tier = digest lines + hub→laptop `contested` down-frame + auto-approve withdrawal once per (file, session); worktrees become `<root>/.mpai/worktrees/<projectId>/<slug>`, branch `mpai/<projectId>/<slug>`, old flat scheme unmigrated (disclose). Explore-agent code map with all file:line seams was produced this session — key anchors: agentDriver.ts:147-195 (awareness MCP server + allowedTools), digestFor server.ts:510-517 (laptop-local scope gap), permissions.ts:82-95 (resolve-contain precedent), repoGroups.ts (client grouping), pulls.ts + Header.tsx:151 + PartyPane.tsx:90-108 (delivery rail), workspace.ts:62-83 (provisioning), tech-debt §2.1.

First docs commit on this branch must fold: this HANDOFF edit + the approved spec + PRD §8.2 "Today" staleness fix (docs/PRD.md:369-371 still describes pre-PR-22 state — projects DO have names/members/lifecycle/entrance since PR #22). All 14 plan tasks executed via soltero-skills:lean-sdd, every task independently reviewed clean; final whole-branch review (opus) → one fix wave (6 items) → scoped re-review: all addressed, no new breakage. Fresh suites at tip: server 545/24 · hub 185/8 · client 320/34, tsc ×3 clean. Live walk passed all 8 steps (both modes).
