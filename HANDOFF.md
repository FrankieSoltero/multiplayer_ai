...
## 🚀 START HERE (session #22 cont.) — §8.8 Awareness/collisions: **PLAN IS FINAL — EXECUTE IT. NO MORE PLAN-REVIEW ROUNDS** (controller adjudication on record in docs/plan-reviews/2026-07-30-awareness-collisions-review.md after 7 council rounds / ~140 findings; the last fix pass closed 24/24 with zero residuals). Branch `feature/awareness-collisions`.

**EXECUTION IN FLIGHT — resume from the ledger `.soltero/lean-sdd/2026-07-30-awareness-collisions/progress.md` (authoritative; trust it + git log over memory).** Landed & reviewed clean: 5, 1, 2a, 2b, 3(+fix), 4(+fix 07c31c7), 6a(+t-rename 47a24f8), 7a (ccbd83d), 6b (e818200 — also closed 7a's harness stubs), 7b (80d6906+d30c0c9 remote-peer fix). ALL 19 implementation tasks complete + reviewed clean (through 10b ec18c47). WALK 11a PASSED live (badge, shares row, digest line + spontaneous agent warning, gate withdrawal with reason, once-per-file, CLOSED-contested) AND found a real defect: turn_end recompute lands after that turn's facts frame — touched one-event-stale in hub mode → Task 4 fix round 2 in flight (routed to the Task 4 implementer; do NOT route fixes to reviewers — twice-made controller error, see ledger). STATE: ALL 21 tasks complete+reviewed; BOTH walks passed (11b confirmed the ordering fix live); Task 12 docs done (bf4330b); FINAL REVIEW returned 0C/1I/4m → fix wave 7402eab landed (reconnect-clear hole closed RED-proven; driverName bound; cost comments; F4 queue-to-sweep reason SKIPPED-disclosed — needs event-log amendment, owner call). Scoped re-review in flight. REMAINING: re-review verdict → fresh suites → update pr-body.md numbers + F4 disclosure → push → OPEN PR (never merge) → plain summary → lesson entry (twice-made reviewer-misroute) → board close. Suites at fc882a6: server 705/30, hub 194, client 346/37, tsc clean, bundle check 1. REMAINING: 10a → 10b → 11a walk → 11b walk → 12 docs → final review → ONE fix wave → PR. Extra execution rulings in ledger: bundle check re-homed to 9b (plan defect); .perm-why pre-existed (8c concern moot); peer-less reason = bare "contested" (ratified+tested). Execution rulings this phase (all in ledger): frame keys on t (spec §8a.6 amended); -uall; remote peers get id-only digest lines (spec §6a union governs brief); lastContestedSent keyed (projectId, sessionId). Key PR disclosures accruing in ledger (legacy mpai/<slug> ref D/F collision; -uall spec touch-up uncommitted in spec §3.1; >512 path drop). Original instruction block follows for the pipeline mechanics:

**Execute `docs/plans/2026-07-30-awareness-collisions.md` via soltero-skills:lean-sdd.** 21 tasks, SERIAL order (verbatim in the plan, tasks are one total exclusion group — no concurrent writers; reviewers may overlap read-only): 5 → 1 → 2a → 2b → 3 → 4 → 6a → 7a → 6b → 7b → 8a → 8p → 8b → 8c → 9a → 9b → 10a → 10b → 11a → 11b → 12. Model policy (same as §8.7): impl mechanical=sonnet else opus; reviews mechanical=haiku SPEC_ONLY, standard=sonnet FULL, judgment=opus FULL; scoped re-reviews sonnet. Controller=fable, never dispatched. Workspace via lean-sdd's scripts/sdd-workspace; ledger progress.md with the fixed line formats; task-brief + review-package scripts per task; fix-loop cap 3; ONE final whole-branch review (opus) + ONE fix wave; then Tasks 11a/11b ARE the walk (controller-driven, Playwright, ports 4000/3002 — NEVER 3001, user's daemon); then push + **OPEN PR, never merge** + plain-language summary.

**Spec of record:** docs/specs/2026-07-30-awareness-collisions-design.md — §8a rulings 1–8 LOCKED (1 scope-split; 2 turn-boundary+decision-site freshness; 3 debt §2.1 root fix; 4 MPAI_CONTESTED_GATE=0 kill switch; 5 touched-broadcast exposure accepted; 6 richer contested frame with collisions[]; 7 old-worktree orphaning ratified; 8 5s sync git latency accepted). Spec governs where plan is silent/wrong. PR must disclose: >512-char paths dropped-and-logged (proposed ruling 9, plan Final-pass notes), old-scheme orphaning, `:memory:`-mode entrance-copy overclaim precedent.

**Review history (full record):** docs/plan-reviews/2026-07-30-awareness-collisions-review.md — cycle 1: 82.3/84.2/83.6 (cap); cycle 2: 83.9/83.6/85.2-with-D6-78 (cap) → adjudicate-and-execute per checkpoint announced to owner. Gate = 0.19.1 severity-driven (≥85 + floors ≥80 + zero blockings) — CC-001 retired (fix shipped upstream; ledger updated).

**§8.7 context:** The record is MERGED (PR #26 → main 6ba1ee2). Demo hub may still be running on port 4000 from the demo (kill only OUR pids, never the user's 3001 daemon; check before binding 4000).

**Gotchas (standing):** never `git add -A` (user WIP: tetris.test.ts + 4 untracked root files); client typecheck `npx tsc -b`; hub/client suites need poc/server BUILT FIRST (plan verifies now build explicitly — pretest hooks don't fire under `npx vitest run`); grep aliased to ugrep (`command grep`); workflow/agent results arrive as notifications — never predict; owner questions go to the USER; walk scratch repos need `git commit --allow-empty -m init` (unborn-HEAD trap, plan pins it).
